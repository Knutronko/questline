...
def test_v5_store_gains_lens_implications_table(tmp_path: Path) -> None:
    """schema_version=5 DB gains lens_implications via migration 6 (G1 live report)."""
    from questline.core.migrations import (
        _migrate_001_initial_core,
        _migrate_002_tests_feature_id,
        _migrate_003_balance_snapshots,
    ...
    assert "lens_implications" not in names
    conn.close()
...
def test_v7_store_gains_agent_tasks_table(tmp_path: Path) -> None:
    """schema_version=7 DB gains agent_tasks via migration 8 (phase-12)."""
    from questline.core.migrations import (
        _migrate_001_initial_core,
        _migrate_002_tests_feature_id,
        _migrate_003_balance_snapshots,
    ...
    assert "agent_tasks" not in names
    conn.close()
